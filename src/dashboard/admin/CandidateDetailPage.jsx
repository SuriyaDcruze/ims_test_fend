        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
              ...
              <p className="text-3xl font-bold text-purple-600">{stats.totalTestAttempts}</p>
            </div>
            <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
              <p className="text-sm text-gray-600 mb-1">Exam Attempts</p>
              <p className="text-3xl font-bold text-orange-600">{stats.totalExamAttempts}</p>
            </div>
          </div>
        </div>
          ...
          <div className="p-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-orange-50 border border-orange-200 rounded-lg p-6">
                      ...
                      <p className="text-4xl font-bold text-orange-600">{timeline.length}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {activeTab === 'courses' && (
              <div className="space-y-4">
                <h3 className="text-xl font-semibold mb-4">Course Purchases & Progress</h3>
                {enrolledCourses.length === 0 ? (
                  <p className="text-gray-500 text-center py-8">No courses purchased</p>
                ) : (
                  enrolledCourses.map((course, index) => (
                    <div key={course.courseId} className="bg-gray-50 border rounded-lg p-6 hover:shadow-md transition-shadow">
            ...
            {activeTab === 'tests' && (
              <div className="space-y-4">
                <h3 className="text-xl font-semibold mb-4">Test Attempts</h3>
                {testResults.length === 0 ? (
                  <p className="text-gray-500 text-center py-8">No test attempts found</p>
                ) : (
                  testResults.map((test, idx) => (
                    <div key={idx} className="bg-white border-l-4 border-blue-500 rounded-lg p-6 shadow-sm">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <h4 className="font-semibold text-lg">{test.sublessonTitle}</h4>
                        ...
                        <div className="text-right">
                          <p className="text-2xl font-bold text-blue-600">{test.bestPercentage}%</p>
                          <p className="text-xs text-gray-500">Best Score</p>
                        </div>
                      </div>
                      <div className="space-y-2">
                        {test.attempts?.slice(0, 5).map((attempt, attemptIdx) => (
                          <div key={attemptIdx} className="flex items-center justify-between bg-gray-50 p-3 rounded text-sm">
                            <div className="flex items-center gap-2">
            ...
            {activeTab === 'exams' && (
              <div className="space-y-4">
                <h3 className="text-xl font-semibold mb-4">Mock Exam Attempts</h3>
                {examResults.length === 0 ? (
                  <p className="text-gray-500 text-center py-8">No exam attempts found</p>
                ) : (
                  examResults.map((exam, idx) => (
                    <div key={idx} className="bg-white border-l-4 border-green-500 rounded-lg p-6 shadow-sm">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <h4 className="font-semibold text-lg">{exam.examTitle}</h4>
                        ...
                        <div className="text-right">
                          <p className="text-2xl font-bold text-green-600">{exam.bestPercentage}%</p>
                          <p className="text-xs text-gray-500">Best Score</p>
                        </div>
                      </div>
                      <div className="space-y-2">
                      ...
                      <div key={index} className="flex gap-4 pb-6 relative">
                        <div className="flex flex-col items-center">
                          <div className={`w-4 h-4 rounded-full border-2 border-white ${
                            item.type === 'course_purchase' ? 'bg-blue-500' :
                          ...
                          <div className="flex justify-between items-start">
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                {item.type === 'course_purchase' && <BookOpen size={16} className="text-blue-500" />}
                                {item.type === 'test_attempt' && <Award size={16} className="text-purple-500" />}
                                {item.type === 'exam_attempt' && <Award size={16} className="text-green-500" />}
